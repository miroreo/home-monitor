<script lang="ts">
	...
	import TransitAlerts from '$lib/transitAlerts.svelte';
...
<main class="flex flex-col sm:flex-row flex-wrap">
    <div class="sm:flex-col flex-row">
        <ElectricPrice price={statusState?.energy?.currentHourPrice} />
        <!-- <TransitAlerts alerts={statusState?.transit?.alerts} /> -->
    </div>
...
<footer class="bottom-0 fixed right-0 p-1 w-auto bg-gray-600 rounded-full m-2.5">
    <div class="align-middle flex flex-row items-center gap-x-4">
        <div class="min-w-50 text-right w">
